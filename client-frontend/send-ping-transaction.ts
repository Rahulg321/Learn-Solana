  ...
  keys: [
    {
      ...
      isSigner: false, // Data account is not a signer
      isWritable: true, // Data account is modified
    },
  ],
});
...
try {
  ...
  if (error instanceof web3.SendTransactionError) {
    ...
    console.log(error.getLogs(connection));
  }

  console.log("an error occured while signing trans", error);
}
